...
#[derive(Accounts)]
#[instruction(bump_moderator_profile: u8, bump_user_profile: u8, bump_big_note: u8, bump_verification_application: u8, bump_verification_pda: u8)]
pub struct RejectBigNoteVerificationApplication<'info> {
...
impl<'info> RejectBigNoteVerificationApplication<'info> {
...
pub fn reject_big_note_verification_application(ctx: Context<RejectBigNoteVerificationApplication>) -> Result<()> {
    ...
    let receiver = &mut ctx.accounts.profile_owner;
    ...
    msg!("Big Note PDA account with address {} not verified", ctx.accounts.big_note.key());
    Ok(())
}
