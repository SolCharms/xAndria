...
pub fn handler(ctx: Context<EditQuestionModerator>, new_title: String, new_tags: Vec<Tags>) -> Result<()> {
    ...
    msg!("Question PDA account with address {} has been edited by moderator with pubkey {}",
         ctx.accounts.question.key(), ctx.accounts.moderator.key());
    Ok(())
}
