...
pub fn handler(ctx: Context<AddContentToQuestion>, new_content: String) -> Result<()> {
    ...
    // ctx.accounts.question.title.serialize(&mut title_buffer).unwrap();
    ...
    // let mut old_content_buffer: Vec<u8> = Vec::new();
    // ctx.accounts.question.content.serialize(&mut old_content_buffer).unwrap();

    // let old_content_buffer_as_slice: &[u8] = old_content_buffer.as_slice();
    // let old_content_buffer_slice_length: usize = old_content_buffer_as_slice.len();
    // let old_content_buffer_end_byte = 128 + title_buffer_slice_length + old_content_buffer_slice_length;

    // let mut tag_buffer: Vec<u8> = Vec::new();
    // ctx.accounts.question.tag.serialize(&mut tag_buffer).unwrap();
